...
module.exports.lastUserWinnerDeclareCall = async (tb) => {
    ...
    const tabInfo = await AviatorTables.findOneAndUpdate(upWh, updateData, { new: true });
    ...
    const tbInfo = await AviatorTables.findOneAndUpdate(dcUWh, up, { new: true });
...
module.exports.winnerDeclareCall = async (winner, tabInfo) => {
    try {
        ...
        const tbInfo = await AviatorTables.findOneAndUpdate(upWh, updateData, { new: true });
        ...
        for (let i = 0; i < tbInfo.gameTracks.length; i++) {
            if (tbInfo.gameTracks[i].playStatus == "win") {
                await walletActions.addWallet(tbInfo.gameTracks[i]._id, Number(winnerTrack.winningAmount), 4, "eviator Win", tabInfo);
            }
        }
