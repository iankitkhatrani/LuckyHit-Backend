...
const userReconnect = async (payload, socket) => {
  try {
    ...
    const disconnTable = await findDisconnectTable(payload.playerId, AviatorTables);
...
const updateRejoinStatus = async (payload, table) => {
  try {
    ...
    if (table) {
      ...
      let tabInfo = await AviatorTables.findOne(wh, project);
      ...
      tabInfo = await AviatorTables.findOneAndUpdate(upWh, updateData, {
        new: true,
      });
